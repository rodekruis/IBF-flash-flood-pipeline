...
class ForcingProcessor:
    def __init__(self, ta_gdf: gpd.GeoDataFrame):
        self.ta_gdf = ta_gdf
        self.current_date_utc = datetime.datetime.now().astimezone(datetime.timezone.utc).replace(tzinfo=None)
    ...
    def most_recent_cosmo_date(self):
        latest_cosmo_date = max(
            [
                datetime.datetime.strptime(date_string.name, "COSMO_MLW_%Y%m%dT%H_prec.nc")
    ...
    def cosmo_prediction_found(self):
        ...
        if self.most_recent_cosmo_date.date() == self.current_date_utc.date():
            return True
        elif self.most_recent_cosmo_date.date() == self.current_date_utc.date() - datetime.timedelta(days=1):
            if self.current_date_utc < self.current_date_utc.replace(hour=7, minute=0, second=0):
                return True
            else:
                return False
        else:
    ...
    def cosmo_date_to_use(self):
        ...
        """
        if self.most_recent_cosmo_date.date() == self.current_date_utc.date():
            return self.current_date_utc.replace(hour=0, minute=0, second=0, microsecond=0)
        elif self.most_recent_cosmo_date.date() == self.current_date_utc.date() - datetime.timedelta(days=1):
            if self.current_date_utc < self.current_date_utc.replace(hour=7, minute=0, second=0):
                return (self.current_date_utc - datetime.timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
            else:
    ...
    def retrieve_forecast(self):
        if self.cosmo_prediction_found:
            ...
            cosmo_path = Path(r"data/cosmo/COSMO_MLW_{}T00_prec.nc".format(self.cosmo_date_to_use.strftime("%Y%m%d")))
    ...
    def construct_forcing_timeseries(self):
        ...
        if last_gpm_timestep.to_pydatetime() >= forcing_forecast.index[0].to_pydatetime():
            ...
            cosmo_path_data_gap = Path(r"data/cosmo/COSMO_MLW_{}T00_prec.nc".format(forcing_gap_start.strftime("%Y%m%d")))
            ...
            if cosmo_path_data_gap.exists():
                ...
                forcing_timeseries_datagap = cosmo_data.loc[
                    (cosmo_data.index > forcing_gap_start) & (cosmo_data.index <= forcing_gap_end)
                ].copy()
                ...
                forcing_timeseries_datagap = forcing_timeseries_datagap.loc[
                    (forcing_timeseries_datagap.index > forcing_gap_start) & (forcing_timeseries_datagap.index <= forcing_gap_end)
                ]
        ...
        forcing_combined.to_csv(rf"data/{ENVIRONMENT}/debug_output/forcing_combined_{datetime.datetime.now().strftime('%Y%m%d-%H')}.csv")

        split_forcing_dfs = [
            forcing_combined[[c]].rename(columns={c: "precipitation"}).reset_index(names="datetime")
